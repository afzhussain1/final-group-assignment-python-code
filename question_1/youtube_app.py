...
class YouTubeApp:
    ...
    @logMethodCall
    def imageClassificationB(a):
        buttonFrame = ttk.Frame(a.root, style="Custom.TFrame")
        buttonFrame.pack(side=tk.TOP, fill=tk.X)
        cButton = ttk.Button(buttonFrame, text="Classify Image", command=a.openImageClassifier)
        cButton.pack(padx=10, pady=10)

    @logMethodCall
    def openImageClassifier(a):
        filePath = filedialog.askopenfilename()
        if filePath:
            result = a.classifyImage(filePath)
            messagebox.showinfo("Classification Result", f"Predicted class: {result}")

    def classifyImage(a, imagePath):
        model = tf.keras.applications.MobileNetV2(weights="imagenet")
        image = Image.open(imagePath).resize((224, 224))
        image = np.array(image) / 255.0
        image = np.expand_dims(image, axis=0)
        predictions = model.predict(image)
        decodedPredictions = tf.keras.applications.mobilenet_v2.decode_predictions(predictions, top=1)
        return decodedPredictions[0][0][1]
...
# import tensorflow as tf
# import re
# from languages import languages  # Import the languages list
...
#     def showTranslation(a):
#         translationWindow = tk.Toplevel(a.root)
#         translationWindow.title("Translated Text")

#         inputLabel = ttk.Label(translationWindow, text="Enter text to translate:")
#         inputLabel.pack(padx=10, pady=5)

#         a.inputText = tk.Text(translationWindow, height=5, width=40)
#         a.inputText.pack(padx=10, pady=5)

#         languageLabel = ttk.Label(translationWindow, text="Select target language:")
#         languageLabel.pack(padx=10, pady=5)

#         a.languageOption = ttk.Combobox(translationWindow, values=languages)
#         a.languageOption.pack(padx=10, pady=5)

#         translateButton = ttk.Button(translationWindow, text="Translate", command=a.translateText)
#         translateButton.pack(padx=10, pady=10)

#     @logMethodCall
#     def translateText(a):
#         text = a.inputText.get("1.0", "end-1c")
#         target_language = a.languageOption.get()

#         if not text.strip():
#             messagebox.showerror("Translation Error", "Please enter text to translate.")
#             return

#         if not target_language:
#             messagebox.showerror("Translation Error", "Please select a target language.")
#             return

#         # call the pre-trained model with the right model name for translation
...
#             # Output
#             resultWindow = tk.Toplevel(a.root)
#             resultWindow.title("Translation Result")

#             resultLabel = ttk.Label(resultWindow, text="Translated Text:")
#             resultLabel.pack(padx=10, pady=5)

#             resultText = tk.Text(resultWindow, height=5, width=40)
#             resultText.pack(padx=10, pady=5)
#             resultText.insert("1.0", translated_text)
...
#         buttonFrame = ttk.Frame(a.root, style="Custom.TFrame")
#         buttonFrame.pack(side=tk.TOP, fill=tk.X)
#         cButton = ttk.Button(buttonFrame, text="Image Classification", command=a.showImageClassification)
#         cButton.pack(padx=10, pady=10)

#     @logMethodCall
#     def showImageClassification(a):
#         classificationWindow = tk.Toplevel(a.root)
#         classificationWindow.title("Image Classification")

#         inputLabel = ttk.Label(classificationWindow, text="Select an image for classification:")
#         inputLabel.pack(padx=10, pady=5)

#         selectButton = ttk.Button(classificationWindow, text="Browse", command=a.selectImage)
#         selectButton.pack(padx=10, pady=10)
...
#         resultWindow = tk.Toplevel(a.root)
#         resultWindow.title("Classification Result")

#         resultLabel = ttk.Label(resultWindow, text="Predicted Label:")
#         resultLabel.pack(padx=10, pady=5)

#         resultText = ttk.Label(resultWindow, text=predicted_label)
#         resultText.pack(padx=10, pady=5)
